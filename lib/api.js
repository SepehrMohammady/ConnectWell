...
// A reply may only point at a live message in the SAME conversation. Anything
// else is refused rather than quietly dropped: silently sending an unthreaded
// message would look like the reply worked. Returns null for "no reply" and
// false for "refuse".
function replyTarget(raw, convId) {
    if (raw === undefined || raw === null || raw === '') return null;
    // Only a number or a numeric string; `true` and `[5]` both survive Number().
    if (typeof raw !== 'number' && typeof raw !== 'string') return false;
    const id = Number(raw);
    if (!Number.isInteger(id) || id <= 0) return false;
    const src = DB.stmts.messageById.get(id);
    // Same-conversation check doubles as the access check: membership of this
    // conversation was already established by the caller.
    if (!src || src.conversation_id !== convId || src.deleted || src.type === 'system') return false;
    return id;
}
...
router.post('/conversations/:id/messages', requireAuth, (req, res) => {
    ...
    const replyTo = replyTarget(req.body?.replyTo, conv.id);
    if (replyTo === false) return res.status(400).json({ error: 'That message cannot be replied to' });
    const message = DB.addMessage({ convId: conv.id, senderId: req.user.id, type: 'text', content, replyTo });
