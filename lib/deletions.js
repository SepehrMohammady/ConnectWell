...
function settle(reqId, state) {
    ...
    if (state === 'approved') {
        ...
        if (m && !m.deleted) {
            ...
            // Same sweep as api.js hardDelete: soft deletes never fire the
            // activity cascade.
            ws.clearMessageActivity(m.id);
            // Removed WITH agreement, so it keeps a tombstone; a delete before
            // anyone saw it leaves nothing behind.
            DB.stmts.markTombstone.run(m.id);
            ws.broadcastToConv(m.conversation_id, {
                t: 'msg:deleted', d: { convId: m.conversation_id, messageId: m.id, tombstone: true },
            });
            // Same reason as api.js hardDelete: the sidebar row is per-viewer.
            for (const row of DB.stmts.convMembers.all(m.conversation_id)) {
                const conv = DB.stmts.convById.get(m.conversation_id);
                if (conv) {
