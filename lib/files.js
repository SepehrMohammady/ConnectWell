...
router.post('/conversations/:id/upload', requireAuth, (req, res) => {
    ...
    // Which message this upload answers, if any. Validated like the text route:
    // a live message in this same conversation, or nothing. The bytes have
    // already crossed the wire, so a bad value drops the threading rather than
    // the file.
    let replyTo = null;
    const rawReply = Number(req.headers['x-reply-to'] || 0);
    if (Number.isInteger(rawReply) && rawReply > 0) {
        const src = DB.stmts.messageById.get(rawReply);
        if (src && src.conversation_id === convId && !src.deleted && src.type !== 'system') replyTo = rawReply;
    }
