...
// The message this one answers. Deliberately NOT a foreign key with a cascade:
// messages soft-delete, and a reply must survive the disappearance of what it
// replied to — the quote then reads as "message deleted" rather than vanishing.
...
const stmts = {
    ...
    insertMessage: db.prepare(
        'INSERT INTO messages (conversation_id, sender_id, type, content, file_name, file_size, mime, duration, created_at, sys_key, sys_args, fwd_from, reply_to) '
        + 'VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)'),
    // Just enough of the quoted message to render the strip above a reply. The
    // body is not stored on the reply: editing or deleting the original must
    // change every quote of it, which only a live lookup gives.
    replySource: db.prepare(
        'SELECT id, sender_id, type, content, file_name, deleted, tombstone FROM messages WHERE id = ?'),
...
// The quoted strip above a reply: who wrote it and just enough to recognise
// it. Returns null when the original is gone entirely, so the quote can say so
// rather than render an empty box.
const QUOTE_MAX = 140;
const clipQuote = (s) => (s && s.length > QUOTE_MAX ? s.slice(0, QUOTE_MAX) + '…' : s);

function replySourceJson(id) {
    const s = stmts.replySource.get(id);
    if (!s) return null;
    const gone = !!s.deleted;
    return {
        id: s.id,
        senderId: s.sender_id,
        type: gone ? 'text' : s.type,
        // A file's caption names it better than its type does; a removed message
        // carries no text at all. Clipped because the quote renders as a single
        // ellipsized line — shipping the whole 4000-character original on every
        // reply, on every page fetch, is payload nobody ever sees.
        content: gone ? null : clipQuote(s.content),
        fileName: gone ? null : s.file_name,
        deleted: gone,
    };
}
...
function messageJson(m) {
    ...
    return {
        ...
        // Resolved live rather than copied at send time, so an edit or a deletion
        // of the original is reflected in every quote of it.
        replySource: m.reply_to ? replySourceJson(m.reply_to) : null,
